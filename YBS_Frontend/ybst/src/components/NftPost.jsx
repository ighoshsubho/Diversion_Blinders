  ...
  const uploadArtToIpfs = async () => {
    try {
      const nftstorage = new NFTStorage({
        token: process.env.NEXT_APP_NFT_STORAGE,
      });
    ...
    try {
      ...
      const response = await axios.post(
        `https://api.nftport.xyz/v0/mints/easy/urls`,
        {
          ...
          description:
            description?.length > 0 ? description : "AI generated NFT",
          mint_to_address:
            address?.length > 0
              ? address
              : "0x831d2b639238973BC16F8Be80911204Ca9e13B58",
        },
        {
          headers: {
            Authorization: process.env.NEXT_APP_NFT_PORT,
          },
        }
      );
